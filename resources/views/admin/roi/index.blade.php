<x-app-layout>
    <div class="container-fluid px-4">
        <div class="d-flex justify-content-between align-items-center my-3">
            <h4 class="mb-0">Potential ROI Inquiries</h4>
        </div>
                        ...
                        <tbody>
                            @forelse($rois as $roi)
                                <tr>
                                    ...
                                    <td class="px-4 fw-semibold text-dark">{{ $roi->industry }}</td>
                                    <td class="px-4">{{ $roi->budget }}</td>
                                    <td class="px-4">{{ $roi->goal }}</td>
                                    <td class="px-4">{{ $roi->business_stage }}</td>
                                    <td class="px-4">{{ $roi->timeline }}</td>
                                    <td class="px-4 text-muted" style="font-size: 0.85rem;">{{ $roi->created_at->format('M d, Y') }}</td>
                                    <td class="px-4 text-end">
                                        <form action="{{ route('admin.roi.delete', $roi->id) }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="text-center text-muted py-5">No inquiries found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
